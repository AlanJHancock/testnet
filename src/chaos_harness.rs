    ...
    if input.chain_id != SYNERGY_TESTNET_V2_CHAIN_ID {
        findings.push(error(
            ...
            format!(
                ...
                SYNERGY_TESTNET_V2_CHAIN_ID, input.chain_id
            ),
        ));
    }
    if input.network_id != SYNERGY_TESTNET_V2_NETWORK_ID {
        findings.push(error(
            ...
            format!(
                ...
                SYNERGY_TESTNET_V2_NETWORK_ID, input.network_id
            ),
        ));
    }
...
mod tests {
    ...
    #[test]
    fn chaos_stop_two_of_six_still_finalizes_with_four_validator_quorum() {
        ...
        assert_eq!(report.clusters[0].quorum_threshold, 4);
    ...
    #[test]
    fn chaos_network_partition_two_of_six_still_finalizes_without_lowering_quorum() {
        let report = run_chaos_harness(&input(
            ChaosScenario::NetworkPartition,
            ChaosBehavior::Finalizes,
    ...
    fn chaos_two_cluster_fixture_reports_independent_dynamic_quorums() {
        ...
        assert_eq!(report.clusters[1].quorum_threshold, 4);
