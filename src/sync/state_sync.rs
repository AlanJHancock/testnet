...
use crate::synergy_types::{ChainId, Hash, NetworkId, SYNERGY_TESTNET_V2_NETWORK_ID};
...
fn validate_apply_plan(plan: &StateSyncRepairPlan, findings: &mut Vec<StateSyncFinding>) {
    ...
    if let Err(detail) = ChainId(plan.chain_id).require_testnet_v2() {
        findings.push(error("state_sync_repair_wrong_chain_id", detail));
    }
    if let Err(detail) = NetworkId(plan.network_id.clone()).require_testnet_v2() {
...
fn validate_request(request: &StateSyncRequest, findings: &mut Vec<StateSyncFinding>) {
    ...
    if let Err(detail) = ChainId(request.chain_id).require_testnet_v2() {
        findings.push(error("state_sync_wrong_chain_id", detail));
    }
    if let Err(detail) = NetworkId(request.network_id.clone()).require_testnet_v2() {
...
fn validate_source_proof(
    ...
    if source.network_id != SYNERGY_TESTNET_V2_NETWORK_ID {
        findings.push(error(
            "state_sync_source_wrong_network_id",
            format!("source network_id must be {SYNERGY_TESTNET_V2_NETWORK_ID}"),
        ));
    }
    for (label, value) in [
