...
pub fn evaluate_fleet_status(snapshot: &FleetStatusSnapshot, strict: bool) -> FleetStatusReport {
    ...
    if snapshot.chain_id != Some(SYNERGY_TESTNET_V2_CHAIN_ID) {
        findings.push(error(
            "chain_id_mismatch",
            format!(
                "expected chain_id {} but snapshot has {:?}",
                SYNERGY_TESTNET_V2_CHAIN_ID, snapshot.chain_id
            ),
        ));
    }
    if snapshot.network_id.as_deref() != Some(SYNERGY_TESTNET_V2_NETWORK_ID) {
        findings.push(error(
            "network_id_mismatch",
            format!(
                "expected network_id {} but snapshot has {:?}",
                SYNERGY_TESTNET_V2_NETWORK_ID, snapshot.network_id
            ),
        ));
    }
...
mod tests {
    ...
    fn valid_snapshot() -> FleetStatusSnapshot {
        FleetStatusSnapshot {
            chain_id: Some(SYNERGY_TESTNET_V2_CHAIN_ID),
            network_id: Some(SYNERGY_TESTNET_V2_NETWORK_ID.to_string()),
            validators: vec![
