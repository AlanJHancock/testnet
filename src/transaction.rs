...
impl Transaction {
    ...
    pub fn has_sufficient_balance(&self, sender_balance: u128) -> bool {
        sender_balance
            >= (self.amount as u128)
                .saturating_add(self.get_max_network_fee_reserve_nwei())
    }
    ...
    pub fn network_fee_breakdown_with_gas(
        ...
        use crate::gas::{
            calculate_network_fee, FeeSchedule, NetworkFeeInput, TransactionFeeType,
            ValuationStatus,
        };
        ...
        let valuation_source = match valuation_status {
            ValuationStatus::NativeSnrg => "native_snrg",
            ValuationStatus::FixedConfig => "testnet_fixed_config",
            ValuationStatus::Unavailable => "unavailable",
            ValuationStatus::NotRequired => "not_required",
        }
        .to_string();
    ...
    fn fee_value_context(
        ...
        if data.starts_with("mint:") {
            let (asset, amount) = parse_asset_amount_payload(data.strip_prefix("mint:"));
            return (
                TransactionFeeType::Mint,
                asset.unwrap_or_else(|| "UNKNOWN".to_string()),
                amount.unwrap_or(self.amount as u128),
                0,
                ValuationStatus::Unavailable,
            );
        ...
        if data.starts_with("ai_job_payment:") {
            let (_, amount) = parse_asset_amount_payload(data.strip_prefix("ai_job_payment:"));
            return (
        ...
        if data.starts_with("sxcp_intent:")
            || data.starts_with("sxcp_proof:")
            || data.starts_with("bridge_transfer:")
            || data.starts_with("cross_chain:")
        {
            return (
