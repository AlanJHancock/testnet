...
mod native_impl {
    ...
    pub fn encaps(pk_bytes: &[u8]) -> (Vec<u8>, Vec<u8>) {
        let pk = match mceliece348864::PublicKey::from_bytes(pk_bytes) {
            Ok(k) => k,
            Err(_) => return (vec![0u8; MCELIECE_CIPHERTEXT_BYTES], vec![0u8; MCELIECE_SHARED_SECRET_BYTES]),
        };
    ...
    mod tests {
        ...
        fn keygen_encaps_decaps_roundtrip() {
            ...
            assert_eq!(ss1, ss2, "shared secrets from both sides of the exchange must match");
        }
        ...
        fn decaps_with_wrong_secret_key_does_not_match() {
            ...
            assert_ne!(ss1, ss2, "decapsulating with an unrelated secret key must not produce the same shared secret");
        }
    }
}
