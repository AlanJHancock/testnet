...
def _copy_with_id(response: dict[str, Any], request_id: Any) -> dict[str, Any]:
...
def _read_json_response(url: str, body: bytes, timeout: float, max_bytes: int) -> Any:
    ...
    try:
        ...
        raise UpstreamError(f"HTTP status {exc.code}") from exc
    except (URLError, TimeoutError, OSError) as exc:
        raise UpstreamError("transport failure") from exc
...
class Router:
    ...
    def _forward(self, request: dict[str, Any], kind: str) -> tuple[dict[str, Any], str]:
        ...
        for state in states:
            ...
            try:
                response = _read_json_response(
                    state.url, body, timeout, self.config.max_response_bytes
                )
                if kind == "read" and _response_is_fail_closed_read(response):
        ...
        if errors:
            raise UpstreamError("all configured upstreams unavailable") from errors[-1]
        raise UpstreamError("all configured upstream circuits are open")

    def _forward_local_url(self, request: dict[str, Any], url: str) -> dict[str, Any]:
        return _read_json_response(
            url, _json_bytes(request), self.config.timeout_seconds, self.config.max_response_bytes
        )
    ...
    def _cache_get(self, request: dict[str, Any]) -> dict[str, Any] | None:
        ...
        with self._lock:
            ...
            return _copy_with_id(entry.response, _request_id(request))

    def _cache_put(self, request: dict[str, Any], response: dict[str, Any]) -> None:
        if not _response_is_success(response):
            return
        key = _cache_key(request)
        with self._lock:
            self._cache[key] = CacheEntry(dict(response), self.clock() + self.config.cache_ttl_seconds)
            self._cache.move_to_end(key)
    ...
    def route_one(self, request: dict[str, Any]) -> dict[str, Any]:
        method = request["method"]
    ...
    def route(self, payload: Any) -> Any:
        ...
        return [self.route_one(item) for item in payload]
